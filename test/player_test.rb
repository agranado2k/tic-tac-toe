...
class PlayerTest < Minitest::Test
  ...
  def setup
    ...
    @humnan_player = CommandLineGames::HumanPlayer.new(@io_interface_stub, @game)
    @computer_player = CommandLineGames::ComputerPlayer.new(@io_interface_stub, @game)
  ...
  def test_player_choice_computer_1
    ...
    assert_equal @computer_player.choice(@next_player, board), 4
  end

  def test_player_choice_computer_2
    board = CommandLineGames::Board.new(["0", "1", "2", "3", "X", "5", "6", "7", "8"], @io_interface_stub)

    refute_equal @computer_player.choice(@next_player, board), 4
  end

  def test_player_choice_human_1
    @io_interface_stub.input = "0"

    assert_equal @humnan_player.choice(@next_player, @board), "0"
  end

  def test_player_choice_human_2
    @io_interface_stub.input = "1"

    assert_equal @humnan_player.choice(@next_player, @board), "1"
  end 
  ...
  def test_handle_player_bad_input_not_number
    ...
    assert_raises RuntimeError do 
      @humnan_player.choice(@next_player, @board)
    end
  end
